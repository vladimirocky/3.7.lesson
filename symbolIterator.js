...
class DispersionArray {
  ...
  [Symbol.iterator]() {
     ...
     */
    (this.current = this.from - this.to), (this.last = this.from + this.to);
    return this;
  }
  next() {
    /**
...
class firstDict extends DispersionArray {}
class secDict extends DispersionArray {}
Object.setPrototypeOf(firstDict, null); 

let secObj = new secDict(1, 10); 
 ...
 * 
 * Ответ:
 * Наследуется итератор родительского класса (DispersionArray).
 * 
 */
